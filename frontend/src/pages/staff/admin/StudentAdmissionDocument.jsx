...
const Line = ({ label, value, labelClassName = 'w-52' }) => (
  <div className="flex items-baseline gap-2">
    <span className={`shrink-0 text-sm text-slate-800 ${labelClassName}`}>{label}</span>
    <span className="min-w-0 flex-1 border-b border-slate-400 px-1 pb-0.5 text-sm font-medium text-slate-900">
      {value || ' '}
    </span>
  </div>
)
