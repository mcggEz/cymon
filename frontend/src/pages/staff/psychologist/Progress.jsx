                    ...
                    <div className="grid grid-cols-3 gap-3 sm:grid-cols-4 mt-4">
                      {MONTHS.map((month) => {
                        const matchingReport = patientReports.find((r) => {
                          ...
                          return p.includes(month.toLowerCase()) && p.includes(String(selectedYear))
                        })

                        return (
                          <div
                            key={month}
                            className="rounded-xl border border-slate-100 bg-slate-50/30 p-3 flex flex-col justify-between items-center min-h-[95px] text-center"
                          >
                            <span className="text-xs font-bold text-slate-800 uppercase tracking-wide">{month.slice(0, 3)}</span>
                            
                            {matchingReport ? (
                              <button
                                onClick={() => handleOpenEdit(matchingReport)}
                                className={`mt-3 w-full text-[10px] font-semibold py-1 rounded cursor-pointer text-center ${
                                  tone[STATUS_META[matchingReport.status]?.tone || 'amber']
                                }`}
                              >
                                {STATUS_META[matchingReport.status]?.label || 'View'}
                              </button>
                            ) : (
                              <button
                                onClick={() => handleOpenAddForm(`${month} ${selectedYear}`)}
                                className="mt-3 w-full text-[10px] font-medium py-1 rounded border border-dashed border-purple-300 text-purple-700 hover:bg-purple-50 cursor-pointer text-center"
                              >
                                + Create
                              </button>
                            )}
