...
function AttendanceSessionForm({ patientId, patientName, program, date, onClose }) {
  ...
  const [studentId, setStudentId] = useState(patientId || '')
  const [studentName, setStudentName] = useState(patientName || '')
  const [programEnrolled, setProgramEnrolled] = useState(program || '')
  ...
  useEffect(() => {
    ...
    setLoading(true)
    api.admin
      .getAttendanceSessionForm(studentId, monthYear)
      .then((d) => {
        if (d.form) {
          setProgramEnrolled(d.form.program_enrolled || '')
          setFacilitatorSig(d.form.facilitator_signature || null)
          setSignoffDate(d.form.signoff_date ? d.form.signoff_date.slice(0, 10) : new Date().toISOString().slice(0, 10))
          setAdditionalNotes(d.form.additional_notes || '')
          ...
          setAttendance([emptyAttendance()])
          setSessions([emptySession()])
          setFacilitatorSig(null)
          setSignoffDate(new Date().toISOString().slice(0, 10))
          setAdditionalNotes('')
        }
      })
      .catch(() => {})
  ...
  const handleSave = async () => {
    ...
    setSaving(true)
    try {
      const cleanAttendance = attendance.filter((r) => r.date && r.status)
      const cleanSessions = sessions.filter((s) => s.date && (s.domain || s.activity))

      await api.admin.saveAttendanceSessionForm({
        patient_id: studentId,
        month_year: monthYear.trim(),
        program_enrolled: programEnrolled,
        facilitator_signature: facilitatorSig,
        signoff_date: signoffDate,
        additional_notes: additionalNotes,
        attendance: cleanAttendance,
        sessions: cleanSessions,
      })
      toast.success('Attendance & Session Record saved successfully.')
      if (onClose) onClose()
    } catch (err) {
    ...
    >
      <div>
        <FormSection eyebrow="01" title="Student Information">
          <div className="grid gap-4 sm:grid-cols-3">
            <FormField label="Name of Student" className="sm:col-span-1">
              {patientId ? (
                <input className={fieldInput} value={studentName} readOnly disabled />
              ) : (
                <select
                  ...
                  onChange={(e) => {
                    ...
                    setStudentId(selId)
                    const selName = patients.find((p) => p.id === selId)?.name || ''
                    setStudentName(selName)
                  }}
                >
            ...
            <FormField label="Program Enrolled">
              ...
              >
                ...
                <option value="Higher Support Needs (HSN)">Higher Support Needs (HSN)</option>
                <option value="Moderate Support Needs (MSN)">Moderate Support Needs (MSN)</option>
                <option value="Low Support Needs (LSN)">Low Support Needs (LSN)</option>
        ...
        <FormSection eyebrow="02" title="Attendance Record">
          {loading ? (
            <div className="text-center py-4 text-xs text-purple-700 font-medium">Loading form details...</div>
          ) : (
            <>
              ...
              <div className="mt-3 flex flex-wrap items-center gap-4">
                <button
                  ...
                  className="rounded-md border border-purple-300 px-3 py-1.5 text-sm font-medium text-purple-700 hover:bg-purple-50 cursor-pointer print:hidden"
                >
                  Add row
                </button>
      ...
      <div>
        <FormSection eyebrow="03" title="Session Log">
          {loading ? (
            <div className="text-center py-4 text-xs text-purple-700 font-medium">Loading session details...</div>
          ) : (
            <>
              <div className="overflow-x-auto">
                <table className="w-full border-collapse text-sm">
          ...
          <div className="max-w-md space-y-3">
            ...
            <FormField label="Additional Notes (optional)">
              <textarea
                ...
                placeholder="Anything else worth recording about this month"
            ...
            <div className="pt-3">
              <button
                type="button"
                onClick={handleSave}
                disabled={saving || !studentId || !monthYear || loading}
                className="w-full rounded-md bg-purple-700 py-3 text-sm font-semibold text-white hover:bg-purple-800 disabled:opacity-50 cursor-pointer"
              >
                {saving ? 'Saving...' : 'Save Record (FO-11)'}
              </button>
            </div>
          </div>
        </FormSection>
